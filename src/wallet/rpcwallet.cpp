...
void SendMoney(const CTxDestination& address, CAmount nValue, CTransactionRef& tx, bool fPoWAlternative = false)
{
    // Check amount
    if (nValue <= 0)
    ...
    if (!pwalletMain->CreateTransaction(scriptPubKey, nValue, tx, reservekey, nFeeRequired, strError, nullptr, ALL_COINS, (CAmount)0, false, fPoWAlternative)) {
...
UniValue sendmany(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "\nSend to multiple destinations. Recipients are transparent or shield CARI addresses.\n"
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The cari address (either transparent or shield) is the key,\n"
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 6) throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"cariaddress\",    (string) The cari address of the transaction.\n"
