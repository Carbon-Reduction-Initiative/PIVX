...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/CARIParams"
    else
        PARAMS_DIR="$HOME/.cari-params"
    fi
fi
...
Failed to fetch the CARI zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
CARI - fetch-params.sh

This script will fetch the CARI zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common CARI zkSNARK parameters. Note that it is
